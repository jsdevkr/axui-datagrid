...
export const gridReducer = (state = initialState, action) => {
  const processor = {
    [TYPES.INIT]: () => {
      ...
      list = action.receivedList.filter(
        item => (item ? !item[action.options.columnKeys.deleted] : false),
      );
    ...
    [TYPES.SET_DATA]: () => {
      // 전달받은 리스트 중에 출력할 리스트를 필터링
      let list = action.receivedList.filter(
        item => (item ? !item[action.options.columnKeys.deleted] : false),
      );
    ...
    [TYPES.FILTER]: () => {
      const checkAll =
        action.filterInfo[action.colIndex] === false
          ? true
