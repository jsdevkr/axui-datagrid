...
export class GridColumnFilterOption extends React.Component<
  iAXDataGridColumnFilterOptionProps,
  iAXDataGridColumnFilterOptionState
> {
  ...
  public componentDidMount() {
    const clientHeight = getInnerHeight(
  ...
  private getOption() {
    ...
    let sIndex: number = Math.floor(
      this.state.scrollTop / this.state.optionItemHeight,
    );
    let eIndex: number =
      Math.floor(this.state.scrollTop / this.state.optionItemHeight) +
    ...
    const paddingTopNode = (
      <div style={{ height: sIndex * this.state.optionItemHeight }} />
    );
        ...
        {Range(sIndex, eIndex).map(i => {
          ...
          return (
            ...
            >
              ...
              <span className={classNames('axd-option-text')}>
                {option.text}
              </span>
            </div>
          );
