...
export function propsToState(props, state) {
  ...
  state.leftHeaderColGroup = state.colGroup.slice(
    0,
    state.options.frozenColumnIndex,
  );
  ...
  if (isArray(state.options.footSum)) {
    ...
    state.footSumTable = makeFootSumTable(
      state.footSumColumns,
      state.colGroup,
      state.options,
    );
  ...
  if (state.options.body.grouping) {
    if (
      'by' in state.options.body.grouping &&
      ...
      state.bodyGroupingTable = makeBodyGroupingTable(
        state.bodyGrouping.columns,
        state.colGroup,
        state.options,
      );
      state.sortInfo = (() => {
      ...
      state.bodyGroupingMap = makeBodyRowMap(
        state.bodyGroupingTable,
        state.options,
      );
    } else {
