...
function makeHeaderTable(_columns, _options) {
    var columns = immutable_1.fromJS(_columns).toJS();
    var table = {
        rows: [],
    };
    var colIndex = 0;
...
function makeBodyRowTable(_columns, _options) {
    ...
    var colIndex = 0;
    var makeRows = function (_columns, depth, parentField) {
        var row = { cols: [] };
        var i = 0;
        var l = _columns.length;
        var colspan = 1;
        var selfMakeRow = function (__columns, __depth) {
            ...
            for (; i < l; i++) {
                var field = __columns[i], colspan_1 = 1;
                if (!field.hidden) {
                    if ('key' in field) {
                        field.colspan = 1;
                        field.rowspan = 1;
                        ...
                        row.cols.push(field);
                        if ('columns' in field) {
                            colspan_1 = makeRows(field.columns, __depth + 1, field);
                        }
        ...
        for (var c = 0, cl = row.cols.length; c < cl; c++) {
            ...
            if (colStartIndex < _frozenColumnIndex) {
                if (colEndIndex <= _frozenColumnIndex) {
                    ...
                    var leftCol = lodash_1.assignWith({}, col), rightCol = lodash_1.assignWith({}, leftCol);
...
function getTableByStartEndColumnIndex(_table, _startColumnIndex, _endColumnIndex) {
    ...
    if ('rows' in _table) {
        _table.rows.forEach(function (row, r) {
            ...
            for (var c = 0, cl = row.cols.length; c < cl; c++) {
                var col = lodash_1.assignWith({}, row.cols[c]);
...
//# sourceMappingURL=_makeData.js.map