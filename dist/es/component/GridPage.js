...
var GridPage = /** @class */ (function (_super) {
    ...
    GridPage.prototype.render = function () {
        ...
        if (!mounted)
            return null;
        return (<div className={classnames_1["default"]('axd-page')} style={{ height: styles.pageHeight }}>
        <div className={classnames_1["default"]('axd-page-buttons')} style={{ width: pageButtonsContainerWidth }}>
          {pageButtons.map(function (button, bi) {
            return <button key={bi} style={{ height: pageButtonHeight, width: button.width || pageButtonHeight }} onClick={function (e) { return onClickPageButton(e, button.onClick); }}>
              <div data-button-svg className={classnames_1["default"](button.className)}/>
            </button>;
        })}
        </div>
      </div>);
