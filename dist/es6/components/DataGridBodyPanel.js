...
const TableBody = ({ sRowIndex, eRowIndex, filteredList, bodyRow }) => (React.createElement("tbody", null, utils_1.arrayFromRange(sRowIndex, eRowIndex).map(li => {
    ...
    if (item) {
        return bodyRow.rows.map((row, ri) => (React.createElement("tr", { key: ri, className: utils_1.classNames(trClassNames) },
            row.cols.map((col, ci) => (React.createElement(DataGridBodyCell_1.default, { key: ci, li: li, ci: ci, col: col, value: filteredList[li][col.key || ''] }))),
            React.createElement("td", null))));
    }
    return null;
})));
const DataGridBodyPanel = props => {
    const { filteredList = [], asideColGroup = [], leftHeaderColGroup = [], visibleHeaderColGroup = [], asideBodyRowData = { rows: [{ cols: [] }] }, leftBodyRowData = { rows: [{ cols: [] }] }, visibleBodyRowData = { rows: [{ cols: [] }] }, panelName, containerStyle = {}, panelScrollConfig = {}, panelLeft = 0, panelTop = 0, styles = {}, } = props;
